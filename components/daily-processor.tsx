...
export const ProcessorProvider = ({ children }: ProcessorProviderProps) => {
    ...
    const processRecords = async () => {
        ...
        let oldHeadlines = headlines.filter((headline) => {
          const headlineDate = new Date(headline.date_processed); // Assuming 'date_processed' is a Date object or a string that can be converted to a Date
    ...
    useEffect(() => {
        if (records.length > 0) {
            processRecords();  // Call processRecords after the records are fetched
        }
    }, [records]);

    return (
    <ProcessorContext.Provider value={{ records, headlines }}>
        {children}
