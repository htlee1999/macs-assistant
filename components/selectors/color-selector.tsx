    ...
    <Popover modal={true} open={open} onOpenChange={onOpenChange}>
      <PopoverTrigger asChild>
        <Button size="sm" className="gap-2 rounded-none" variant="ghost">
          <span
            className="rounded-sm px-1 text-foreground"
            style={{
        ...
        <div className="flex flex-col">
          ...
          {TEXT_COLORS.map(({ name, color }) => (
            ...
            >
              ...
              {editor.isActive("textStyle", { color }) && <Check className="size-4" />}
            </EditorBubbleItem>
          ))}
        </div>
        <div>
          <div className="my-1 px-2 text-sm font-semibold text-foreground">Background</div>
